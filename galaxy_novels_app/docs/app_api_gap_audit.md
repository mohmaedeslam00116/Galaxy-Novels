...
نربط `GET /me/novels/{novel_id}` بتفاصيل الرواية لعرض حالة المستخدم الخاصة بها، ثم ننفذ قراءة التقييم الشخصي وإرساله عبر `GET/POST /ratings/novel/{id}`. يبقى `GET /me/history/novel/{id}` اختياريا عند الحاجة إلى شاشة تفاصيل سجل الرواية، وليس مطلوبا لصفحة المتابعة الحالية.
