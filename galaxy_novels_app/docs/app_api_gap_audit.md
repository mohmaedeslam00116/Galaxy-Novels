...
1. جعل أكمل القراءة يعتمد السجل المحلي عندما لا توجد جلسة.
2. جعل بطاقات آخر التحديثات في وضع القائمة قابلة للفتح.
3. تحويل قائمة الفصول الطويلة إلى بناء كسول أو pagination؛ الشاشة الحالية تنشئ كل عناصر الفصول داخل `Column`.
4. تفعيل وجهات Drawer أو إخفاء غير الجاهز منها؛ حسابي وإعدادات القراءة يعملان، وبقية الوجهات تغلق القائمة دون نتيجة.
5. مدير التنزيل يستمر عند التنقل داخل التطبيق، لكنه لا يستعيد مهمة شبكة جارية بعد قتل عملية التطبيق أو إعادة تشغيل الهاتف.
6. اختيار ثيم التطبيق مجهز بالتوكنز فقط، ولا توجد شاشة أو persistence بعد.
...
نربط «أكمل القراءة» بالسجل المحلي، ونجعل عناصر «آخر التحديثات» تفتح تفاصيل الرواية أو الفصل المناسب فعليا.
