...
| تفاصيل الرواية | منفذ جزئيا | البيانات والقراءة والتنزيل والمفضلة وقائمة الفصول الكسولة والبحث المحلي تعمل؛ التقييمات والتعليقات وVIP غير موصولة. |
...
|---|---|---|
| الجلسة | `POST /auth/login`, `POST /auth/logout`, `GET /session`, `GET /me` | منفذ باستثناء `/me`. يعمل الدخول والخروج واستعادة `/session`، ويحفظ خيار "تذكرني" الكوكيز والnonce فقط داخل `flutter_secure_storage`. يعاد nonce مرة واحدة عند انتهاءه أثناء الخروج، ويمسح `401` الجلسة المحلية. |
...
| نشاط القراءة وXP | `POST /reading/sync` | منفذ جزئيا. القارئ يقيس foreground open time والنشاط لمدة 60 ثانية بعد التفاعل، ويحفظ أحداثا ثابتة معزولة لكل user ID، ويرسل حتى 50 حدثا في الدفعة مع منع التكرار وoffline retry. عرض XP المحدث في الحساب غير منفذ بعد. |
...
نربط `GET /me` أو تحديث payload الجلسة بواجهة الحساب لعرض XP والرتبة القادمين من السيرفر بعد مزامنة القراءة، من دون حسابهما محليا. يبقى `GET /me/history/novel/{id}` اختياريا عند الحاجة إلى شاشة تفاصيل سجل الرواية، وليس مطلوبا لصفحة المتابعة الحالية.
