...
تم التحقق باستخدام User-Agent التطبيق العام `WorReaderApp/1.0`:
...
| الرئيسية | منفذ | الرفوف تعمل، وأكمل القراءة يعتمد أحدث سجل محلي. الضغط على بطاقة «آخر التحديثات» يفتح صفحة تفاصيل الرواية، ولا يفتح الفصل مباشرة. |
...
| الإعلانات | منفذ محليا على Android | AdMob مضاف: debug/profile يستخدمان وحدة Google الاختبارية، وAndroid release يستخدم App ID ووحدة الإنتاج بعد بوابة موافقة UMP للبانر داخل القارئ. إعلانات iOS معطلة مؤقتا ولا يهيئ التطبيق SDK عليها. المتبقي قبل النشر هو إكمال إعدادات المتجر والخصوصية و`app-ads.txt`. |

## مصفوفة REST الخاص

| المجال | Endpoints | حالة التطبيق |
|---|---|---|
| الجلسة | `POST /auth/login`, `POST /auth/logout`, `GET /session`, `GET /me` | منفذ. يعمل الدخول والخروج واستعادة `/session` وتحديث الملف من `/me`. يحفظ خيار "تذكرني" `access_token` فقط داخل `flutter_secure_storage`. يرسل التطبيق `Authorization: Bearer` و`X-Wor-App-Token` في الطلبات الخاصة، ولا يرسل WordPress cookies أو `X-WP-Nonce`. يحمي التحديث من تبديل الجلسة، ويدمج طلبات الملف المتزامنة مع cooldown مدته 60 ثانية. مسارات التعليقات والتقييم والمفضلة وVIP تعرض رسالة موحدة عند انتهاء الجلسة مع توجيه المستخدم إلى حسابي حيث يلزم. |
...
1. يجب إكمال إعدادات AdMob قبل النشر: ربط التطبيق بصفحة Google Play بعد توفرها وإعداد `app-ads.txt`. موافقة UMP مطبقة على Android، بينما إعلانات iOS معطلة إلى أن تجهز في إصدار مستقل.
2. لا تظهر داخل التطبيق إجراءات إنشاء حساب أو استعادة كلمة المرور؛ لا توجد endpoints موثقة لها في app API الحالي.
...
- VIP content لا يجوز حفظه محليا دون سياسة انتهاء وحذف مرتبطة بالاشتراك. التطبيق الحالي يقرأ فصول VIP فقط ولا يخزنها بشكل دائم.
...
1. حفظ إعدادات القارئ وربط شاشة الإعدادات. (مكتمل)
2. ربط أكمل القراءة وآخر التحديثات بالتنقل الحقيقي. (مكتمل)
3. تحسين أداء قائمة الفصول الطويلة والبحث داخلها. (مكتمل)
...
6. إضافة إعلان القارئ. (مكتمل، مع وحدة إنتاج في release ووحدة اختبار في debug/profile)

### المرحلة 5: VIP والمتجر

تأتي بعد استقرار الجلسة والمزامنة. فصول VIP للقراءة فقط منفذة الآن: القائمة وحالة الحساب وفتح الفصل داخل القارئ Native تعمل عبر مسار القالب الجديد. سياسة كاش VIP القصير والدفع مؤجلة.
