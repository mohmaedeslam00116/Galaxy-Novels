...
1. تحويل قائمة الفصول الطويلة إلى بناء كسول أو pagination؛ الشاشة الحالية تنشئ كل عناصر الفصول داخل `Column`.
2. تفعيل وجهات Drawer أو إخفاء غير الجاهز منها؛ حسابي وإعدادات القراءة يعملان، وبقية الوجهات تغلق القائمة دون نتيجة.
3. مدير التنزيل يستمر عند التنقل داخل التطبيق، لكنه لا يستعيد مهمة شبكة جارية بعد قتل عملية التطبيق أو إعادة تشغيل الهاتف.
4. اختيار ثيم التطبيق مجهز بالتوكنز فقط، ولا توجد شاشة أو persistence بعد.
...
نحسن أداء قائمة الفصول الطويلة، ونضيف بحثا محليا داخل الفصول دون تحميل إضافي من السيرفر.
