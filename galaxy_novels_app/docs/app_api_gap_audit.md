...
| تفاصيل الرواية | منفذ جزئيا | البيانات والقراءة والتنزيل والمفضلة والتقييم الشخصي وقائمة الفصول الكسولة والبحث المحلي وتبويب التعليقات العامة وكتابة التعليقات والردود والتصويت والتفاعلات تعمل. يظهر تبويب VIP عند وجود جدول خاص، ويعرض رسالة دخول/اشتراك أو قائمة الفصول الخاصة حسب حالة الحساب. فتح أول فصل VIP محمي حاليا لأن مسار `vip/chapter` غير موجود على الإنتاج. |
...
| VIP schedule العام | منفذ جزئيا | الرابط الموجود في بيانات الرواية يستخدم لإظهار تبويب VIP. القائمة نفسها تأتي من المسار الخاص `vip/chapters` وتتطلب Bearer token. |
...
| VIP الخاص | chapters وcontinuous-next وlibrary-counts | منفذ جزئيا. `vip/chapters` موصول عبر `PrivateVipRepository` ويظهر في تفاصيل الرواية للمستخدم المصرح له. `vip/continuous-next` موصول داخل طبقة القارئ native عبر مسار داخلي `vip:next:<id>`. فتح أول فصل مباشرة ينتظر إضافة `vip/chapter` في القالب؛ حاليا الواجهة تعرض رسالة واضحة بدلا من فتح مسار مكسور. لا توجد تنزيلات VIP ولا دفع داخل التطبيق. |
...
- VIP content لا يجوز حفظه مثل الفصول العامة دون سياسة انتهاء وحذف مرتبطة بالاشتراك. التطبيق الحالي لا يخزن فصول VIP ولا يضيفها إلى التنزيلات.
- ملاحظة تنفيذ VIP: على الإنتاج بتاريخ 26 يونيو 2026 كان `vip/chapter` يرجع `rest_no_route`، بينما `vip/chapters` و`vip/continuous-next` موجودان ويرجعان `401` دون Bearer token. لذلك قراءة أول فصل VIP مباشرة داخل التطبيق تحتاج alias/endpoint إضافي في القالب.
...
5. عرض VIP schedule العام. (مكتمل جزئيا: يظهر تبويب VIP وقائمة الفصول الخاصة، مع حارس لغياب `vip/chapter`)

### المرحلة 5: VIP والمتجر

تأتي بعد استقرار الجلسة والمزامنة. فصول VIP للقراءة فقط منفذة جزئيا الآن: القائمة وحالة الحساب والربط الداخلي بالقارئ جاهزة، لكن فتح أول فصل ينتظر مسار `vip/chapter` من القالب. سياسة الكاش القصير والاشتراك والدفع مؤجلة. نظام النقاط والإعلانات ليس جزءا من هذه المرحلة تلقائيا.

## الخطوة التالية مباشرة

الخطوة التالية الأقرب في VIP هي إضافة مسار `GET /wp-json/wor-reader-app/v1/vip/chapter?chapter_id=ID` في القالب حتى يمكن فتح أول فصل VIP مباشرة داخل القارئ Native. يبقى `GET /me/history/novel/{id}` اختياريا عند الحاجة إلى شاشة تفاصيل سجل الرواية، وليس مطلوبا لصفحة المتابعة الحالية.
