...
| الرئيسية | منفذ جزئيا | الرفوف تعمل، لكن أكمل القراءة العام غير موصول بالسجل المحلي، وعنصر آخر التحديثات في وضع القائمة لا يفعل شيئا عند الضغط. |
...
| تفاصيل الرواية | منفذ جزئيا | البيانات والفصول والقراءة والتنزيل تعمل؛ المفضلة والتقييمات والتعليقات وVIP غير موصولة. |
...
| إعدادات القارئ | مؤقتة | تضيع عند إغلاق شاشة القارئ، ولا تعمل وجهة الإعدادات في القائمة الجانبية. |
...
1. حفظ إعدادات القارئ محليا وربط وجهة إعدادات القراءة في Drawer.
2. جعل أكمل القراءة يعتمد السجل المحلي عندما لا توجد جلسة.
3. جعل بطاقات آخر التحديثات في وضع القائمة قابلة للفتح.
4. تحويل قائمة الفصول الطويلة إلى بناء كسول أو pagination؛ الشاشة الحالية تنشئ كل عناصر الفصول داخل `Column`.
5. تفعيل وجهات Drawer أو إخفاء غير الجاهز منها؛ حاليا حسابي فقط يفتح شاشة، وبقية الوجهات تغلق القائمة دون نتيجة.
6. مدير التنزيل يستمر عند التنقل داخل التطبيق، لكنه لا يستعيد مهمة شبكة جارية بعد قتل عملية التطبيق أو إعادة تشغيل الهاتف.
7. اختيار ثيم التطبيق مجهز بالتوكنز فقط، ولا توجد شاشة أو persistence بعد.
...
2. حفظ إعدادات القارئ وربط شاشة الإعدادات.
3. ربط أكمل القراءة وآخر التحديثات بالتنقل الحقيقي.
4. تحسين أداء قائمة الفصول الطويلة والبحث داخلها.
...
نبدأ بحفظ إعدادات القارئ محليا وربط وجهة إعدادات القراءة في Drawer. يجب أن يبقى حجم الخط وتباعد الأسطر ووضع القراءة محفوظا بعد إغلاق القارئ وإعادة تشغيل التطبيق.
