...
نبدأ بعرض التعليقات العامة للرواية والفصل عبر `GET /comments/{type}/{id}` مع pagination وحالة فارغة وفشل غير حاجب. بعد استقرار القراءة العامة نضيف إنشاء التعليقات والردود للمستخدم المسجل. يبقى `GET /me/history/novel/{id}` اختياريا عند الحاجة إلى شاشة تفاصيل سجل الرواية، وليس مطلوبا لصفحة المتابعة الحالية.
