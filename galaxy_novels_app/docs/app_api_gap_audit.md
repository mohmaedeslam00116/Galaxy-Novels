...
| سجل القراءة | منفذ جزئيا | يسجل آخر فصل محليا ويدمج الأحدث لكل رواية مع `GET /me/history` عند وجود جلسة حساب. رفع أحداث القراءة إلى `/reading/sync` غير منفذ. |
...
نبني queue محلية دائمة لأحداث القراءة، مع معرف حدث فريد وقياس `active_seconds` و`open_seconds`، ثم نرسلها على دفعات إلى `POST /reading/sync`. بعد ذلك نعرض XP القادم من السيرفر من دون حسابه محليا. يبقى `GET /me/history/novel/{id}` اختياريا عند الحاجة إلى شاشة تفاصيل سجل الرواية، وليس مطلوبا لصفحة المتابعة الحالية.
